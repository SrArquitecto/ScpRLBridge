...
namespace ScpAgent.Bot.Sensors.Modules
{
    public class VelocityModule : ISensorVelocityModule
    {
        ...
        private const float MAX_ANGULAR_VELOCITY = 360f; // grados/segundo máximo
        ...
        public void Actualizar(AgentObservation obs, SensorContext ctx)
        {
            ...
            float angVelYaw   = 0f;
            float angVelPitch = 0f;
            
            if (ctx.Delta > 0f)
            {
                // Calcular velocidad en grados/segundo y normalizar a [-1, 1]
                angVelYaw   = (deltaYaw / ctx.Delta) / MAX_ANGULAR_VELOCITY;
                angVelPitch = (deltaPitch / ctx.Delta) / MAX_ANGULAR_VELOCITY;
                
                // Clamp para evitar valores fuera de rango
                angVelYaw   = Mathf.Clamp(angVelYaw, -1f, 1f);
                angVelPitch = Mathf.Clamp(angVelPitch, -1f, 1f);
            }
